---
name: qa-refactor-go
description: "[qa] 완료된 Go 리팩터링을 검증합니다. 동작 보존, 계약 유지, 응집도, 과분리 여부를 확인해야 할 때 사용합니다."
---
...
- For implementation:
  - 이 요청이 이 스킬 범위가 아닌 이유를 한 문장으로 말합니다.
  - 더 맞는 스킬이나 조건: execute-refactor-go
...
- Do not fail a refactor because code stayed in one file or package. Fail it only if cohesion is accidental, behavior is harder to verify, or coupling is worse.
- Flag over-splitting when the diff adds files, exported names, interfaces, fixtures, or dependency edges without a clear behavior-preserving clarity gain.
- Do not edit the refactor while verifying it. If a fix is needed, report fail and hand off to `execute-refactor-go` or `execute-bounded-change`.
...
- Prefer the narrowest Go test or command that exercises the touched path.
