...
| Remote runner | Not shipped in AMA iOS; execute only through an approved remote provider |

### How to Reproduce

On AMA iOS, do not run local Python or shell commands. Use `scientific.remote_job_preflight` to prepare a remote TimesFM job, then attach the input artifact and expected output schema to the approved remote provider.
